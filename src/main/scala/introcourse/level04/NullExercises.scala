...
object NullExercises {
  ...
  def changeName(newName: String, person: Person): Person = if (person == null) {
    null
  } else {
    mkPersonOrNull(newName, person.age)
  }
  /**
    * Thought exercise: Does the following function return a `null`?
    */
    // it didn't let me return null! Why?!
  def mean(nums: List[Int]): Double = nums match {
      case ::(_, _) => nums.sum.toDouble / nums.length
      case Nil => 0
    }
